...
import (
	...
	apimanagerevents "github.com/Pickausernaame/chat-service/tests/e2e/api/manager/events"
	apimanagerv1 "github.com/Pickausernaame/chat-service/tests/e2e/api/manager/v1"
)
...
func (ws *Workspace) ReceiveNewProblemsAvailability(ctx context.Context) error {
	...
	ws.setCanTakeMoreProblemsFlag(pointer.Indirect(data.Available))
	return nil
}
...
func (ws *Workspace) SendMessage(ctx context.Context, chatID types.ChatID) error {
	_, ok := ws.getChat(chatID)
	if !ok {
		return fmt.Errorf("%v: %v", errUnknownChat, chatID)
	}
	...
	resp, err := ws.api.PostSendMessageWithResponse(ctx,
		...
		apimanagerv1.SendMessageRequest{
			ChatId:      chatID,
			MessageBody: body,
		},
	)
...
func (ws *Workspace) HandleEvent(_ context.Context, data []byte) error {
	...
	case string(apimanagerevents.BaseEventEventTypeNewChatEvent):
		vv := &apimanagerevents.NewChatEvent{}
	...
	case string(apimanagerevents.BaseEventEventTypeNewMessageEvent):
		vv := &apimanagerevents.NewMessageEvent{}
		if err := json.Unmarshal(data, vv); err != nil {
			return fmt.Errorf("unmarshal event: %v", err)
		}
	...
	case string(apimanagerevents.BaseEventEventTypeChatClosedEvent):
		vv := &apimanagerevents.ChatClosedEvent{}
		if err := json.Unmarshal(data, vv); err != nil {
			return fmt.Errorf("unmarshal event: %v", err)
		}

		if err := ws.removeChat(vv.ChatId); err != nil {
			return fmt.Errorf("removing chat: %v", err)
		}

		ws.canTakeMoreProblems.Store(true)
	}
