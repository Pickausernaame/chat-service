...
func (m *ProblemMutation) SetResolveAt(t time.Time) {
	m.resolve_at = &t
}

// ResolveAt returns the value of the "resolve_at" field in the mutation.
func (m *ProblemMutation) ResolveAt() (r time.Time, exists bool) {
	v := m.resolve_at
	if v == nil {
		return
	}
...
// OldResolveAt returns the old "resolve_at" field's value of the Problem entity.
// If the Problem object wasn't provided to the builder, the object is fetched from the database.
// An error is returned if the mutation operation is not UpdateOne, or the database query fails.
func (m *ProblemMutation) OldResolveAt(ctx context.Context) (v time.Time, err error) {
	if !m.op.Is(OpUpdateOne) {
		return v, errors.New("OldResolveAt is only allowed on UpdateOne operations")
	}
	if m.id == nil || m.oldValue == nil {
		return v, errors.New("OldResolveAt requires an ID field in the mutation")
	}
	oldValue, err := m.oldValue(ctx)
	if err != nil {
		return v, fmt.Errorf("querying old value for OldResolveAt: %w", err)
	}
	return oldValue.ResolveAt, nil
}

// ClearResolveAt clears the value of the "resolve_at" field.
func (m *ProblemMutation) ClearResolveAt() {
	m.resolve_at = nil
	m.clearedFields[problem.FieldResolveAt] = struct{}{}
}

// ResolveAtCleared returns if the "resolve_at" field was cleared in this mutation.
func (m *ProblemMutation) ResolveAtCleared() bool {
	_, ok := m.clearedFields[problem.FieldResolveAt]
	return ok
}

// ResetResolveAt resets all changes to the "resolve_at" field.
func (m *ProblemMutation) ResetResolveAt() {
	m.resolve_at = nil
	delete(m.clearedFields, problem.FieldResolveAt)
}
...
func (m *ProblemMutation) Fields() []string {
	...
	if m.resolve_at != nil {
		fields = append(fields, problem.FieldResolveAt)
	}
	if m.created_at != nil {
...
func (m *ProblemMutation) ClearedFields() []string {
	...
	if m.FieldCleared(problem.FieldResolveAt) {
		fields = append(fields, problem.FieldResolveAt)
	}
	return fields
}
